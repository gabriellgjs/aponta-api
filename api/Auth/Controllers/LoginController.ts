...
export default class LoginController {
  ...
  private async comparePassword(password_hash: string, password_request: string) {
  ...
  public async login(request: Request, response: Response) {
    ...
    const passwordIsMatch = await this.comparePassword(userExists.password, userFactory.password);

    if(!passwordIsMatch) throw new Error("Email ou senha inválidos.")

    return response.status(200).send(this.generateTokenAuthenticationByUser(userExists));
  }
