...
import CreatePermissionAction from '@src/Permissions/Application/Actions/CreatePermissionAction';
import DeletePermissionAction from '@src/Permissions/Application/Actions/DeletePermissionAction';
import UpdatePermissionAction from '@src/Permissions/Application/Actions/UpdatePermissionAction';
import {
  InternalServerError,
  NotFoundError
...
export default class PermissionsController {
  ...
  public async getPermissions(request: Request, response: Response) {
    try {
      ...
      return response.status(200).json(PermissionsOutputData.responseGetPermissions(permissions));
    } catch (error) {
      if (error instanceof InternalServerError)
        throw new InternalServerError(error.message);
    }
  ...
  public async createPermission(request: Request, response: Response) {
    try {
      ...
      const permissionId = (await permissionAction.execute(permissionFactory))?.id;
  ...
  public async updatePermission(request: Request, response: Response) {
    try {
      ...
      const actualPermission = await permissionsModel.getPermission(userDataInput.id);

      const actualPermissionInput = UpdatePermissionFactory.fromCurrentPermission(actualPermission);
